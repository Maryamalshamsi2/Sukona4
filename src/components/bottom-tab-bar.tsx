...
type Icon = "home" | "calendar" | "catalog" | "more" | "users" | "team" | "receipt" | "package" | "chart" | "payroll" | "reviews" | "sales" | "gift" | "settings" | "logout";
...
const MORE_ITEMS: MoreItem[] = [
  ...
  // /payroll is owner-only AND plan-gated (Team+ via canUsePayroll).
...
function NavIcon({ icon, className }: { icon: Icon; className?: string }) {
  ...
  switch (icon) {
    ...
    case "payroll":
      // Same banknote/cash icon used in the desktop sidebar — keeps
      // visual identity consistent between the two nav surfaces.
      return (
        <svg className={cls} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
    ...
    case "reviews":
      // Shield-check — matches the sidebar variant.
      return (
        <svg className={cls} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
    ...
    case "sales":
      // Shopping bag — matches the sidebar variant.
      return (
        <svg className={cls} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
